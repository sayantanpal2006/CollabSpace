...
import com.collabspace.entity.FileAttachment;
import com.collabspace.repository.FileAttachmentRepository;
import com.collabspace.service.CurrentUserService;
...
public class FileController {
    ...
    public Map<String, Object> upload(@RequestParam MultipartFile file) {
        ...
        if (!TYPES.contains(file.getContentType())) throw new IllegalArgumentException("Unsupported file type");
        ...
        try {
            ...
            if (!target.startsWith(root)) throw new IllegalArgumentException("Invalid filename");

            Files.copy(file.getInputStream(), target, StandardCopyOption.REPLACE_EXISTING);

            FileAttachment a = files.save(FileAttachment.builder()
                    .uploader(current.get())
    ...
    public ResponseEntity<Resource> download(@PathVariable UUID id) throws Exception {
        FileAttachment a = files.findById(id).orElseThrow();
        Path p = Paths.get(dir).resolve(a.getStoredName()).normalize();
